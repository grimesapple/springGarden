	...
	<el-container>
		<el-header class="header-wrapper">
			...
			<div class="login-dialog">
				<el-drawer :visible.sync="showLogin" width="30%">
			...
			<div class="hc-home-search-wrapper animate__animated " :class="!showLogin?'animate__bounceIn':''">
				...
				<section class="keyword-search-wrapper">
					...
					<section class="hc-search-keyword-panel" v-if="keywordToggle">
						<section class="hc-search-group-tabs">
							<ul class="search-group-tab-list">
								<li class="search-group-tab-list-item" :class="{ active:currentRegionTypeIndex == 0}"
									@click="getRegionAsync(currentCityName,'hotsearch'),currentRegionTypeClass(0)">热门搜索
								</li>
								<li class="search-group-tab-list-item" :class="{ active:currentRegionTypeIndex == 1}"
									@click="getRegionAsync(currentCityName,'scenicspot'),currentRegionTypeClass(1)">景点
								</li>
								<li class="search-group-tab-list-item" :class="{ active:currentRegionTypeIndex == 2}"
									@click="getRegionAsync(currentCityName,'tradingarea'),currentRegionTypeClass(2)">商圈
								</li>
								<li class="search-group-tab-list-item" :class="{ active:currentRegionTypeIndex == 3}"
									@click="getRegionAsync(currentCityName,'metroline'),currentRegionTypeClass(3)">地铁站
								</li>
								<li class="search-group-tab-list-item" :class="{ active:currentRegionTypeIndex == 4}"
									@click="getRegionAsync(currentCityName,'housetype'),currentRegionTypeClass(4)">房型
								</li>
								<li class="search-group-tab-list-item" :class="{ active:currentRegionTypeIndex == 5}"
									@click="getRegionAsync(currentCityName,'district'),currentRegionTypeClass(5)">行政区
								</li>
								<li class="search-group-tab-list-item" :class="{ active:currentRegionTypeIndex == 6}"
									@click="getRegionAsync(currentCityName,'airportorstation'),currentRegionTypeClass(6)">
									机场车站</li>
								<li class="search-group-tab-list-item" :class="{ active:currentRegionTypeIndex == 7}"
									@click="getRegionAsync(currentCityName,'universities'),currentRegionTypeClass(7)">高校
								</li>
								<li class="search-group-tab-list-item" :class="{ active:currentRegionTypeIndex == 8}"
									@click="getRegionAsync(currentCityName,'hospital',currentRegionTypeClass(8))">医院
								</li>
							</ul>
							<ul class="group-list">
								<li class="group-item-list-item" v-for="(item,key) in region"
									@click="currentRegion=item.name,keywordToggle=false"><a
										class="group-item-name">{{item.name}}</a></li>
							</ul>
						</section>
					</section>
				</section>
...
<style scoped>
	...
	.isActive {
		color: #333;
		cursor: default;
		border-bottom: 2px solid #349307;
