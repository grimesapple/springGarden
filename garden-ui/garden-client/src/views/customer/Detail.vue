				...
				<div class="unit-navbar__container">
					<div class="unit-navbar__container__main">
						<ul class="unit-navbar__nav clearfix">
							<li class="unit-navbar__nav__item"><a :class="navIndex==0?'item--select':''"
									@click="navIndex=0,scroll(810)">房屋介绍</a></li>
							<li class="unit-navbar__nav__item"><a :class="navIndex==1?'item--select':''"
				...
				<div class="detail-container__main">
						...
						<div class="unit-house-info">
							<ul class="clearfix">
								...
								<li>
									...
									<span>{{houseData.house.bedNumber}}张床</span>
								</li>
								<li>
									<i class="icon-yizhujiren"></i>
									<span>宜住{{houseData.house.people}}人</span>
								</li>
								<li>
					...
					<div class="unit-layout__item">
						...
						<div class="unit-description">
							<div v-html="houseData.house.content"></div>
						</div>
					</div>
					<div class="unit-layout__item">
						<h5>设施服务</h5>
					...
					<div class="unit-comment">
						...
						<div class="unit-comment__container">
							<div class="unit-comment__container__rate clearfix">
								<span>综合评分：</span>
								<el-rate v-model="houseData.score" disabled show-score text-color="#ff9900"
									score-template="{value}" style="width: 150px; float: left">
								</el-rate>
								<span>分</span>
							</div>
							<!--评论列表-->
				...
				<div class="unit-price">
					<div class="unit-price__price">
						<span class="unit-price__price--final"><span>￥</span><span
								class="price__count">{{houseData.house.promotePrice}}</span>每晚</span>
					</div>
	...
	export default {
		...
		async created() {
			...
			this.pickerOptions = {
				...
				disabledDate(time) {
					// if (time.getTime() < Date.now() || time.getTime() < (new Date(_this.houseData.chooseDate[0]) -
					// 		8.64e7) ||	
		...
		methods: {
			...
			async getOrder() { //得到房屋已存在的订单
				const _this = this
				await axios.get(this.API.GetOrderByTitle + this.houseData.username + "/" + this.houseData
					.title).then(
					resp => {
			...
			getComments() {
				...
				axios.get(this.API.GetCommentList + this.houseData.username + '/' +
					this.houseData.title + '/' + this.currentPage + '/' + this.pageSize).then(function(
					resp) {
			...
			//得到房间的预定列表
			getTimeList(id) {
				axios.get(this.API.GetHouseTimeList, {
					params: {
						productId: id
					}
				}).then(res => {
