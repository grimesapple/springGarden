...
<template>
	<article style="height: 100%">
		...
		<div class="search" v-if="operation=='seeAll'">
			...
			<span>所在城市：</span>
			<el-input placeholder="请输入内容" v-model="city" style="width: 150px" clearable>
			</el-input>
			<span>用户名：</span>
			<el-input placeholder="请输入内容" v-model="username" style="width: 150px" clearable>
			</el-input>
			<span>房屋名称：</span>
		...
		<!--内容区域-->
		<el-table :data="tableData" height="90%" border>
			<el-table-column prop="id" label="房屋id" width="70" align="center"></el-table-column>
			<el-table-column prop="name" label="房屋名字" width="100" align="center"></el-table-column>
			<el-table-column prop="subTitle" label="房屋标题" width="100" align="center"></el-table-column>
			<el-table-column prop="stock" label="总数量" width="100" align="center"></el-table-column>
			<el-table-column prop="orignalPrice" label="原价格(晚/元)" width="150" align="center"></el-table-column>
			<el-table-column prop="promotePrice" label="促销价格(晚/元)" width="150" align="center"></el-table-column>
			
<!-- 			<el-table-column label="房屋照片" width="180" align="center">
...
<!-- 			<el-table-column prop="username" label="用户" width="120" align="center">
			...
			<el-table-column prop="cityAndRegion" label="房屋位置" width="180" align="center">
			</el-table-column>
			<el-table-column label="配套列表" width="180" align="center">
				<template slot-scope="scope">
					<span>{{scope.row.matchingList.toString()}}</span>
				</template>
			</el-table-column>
			<el-table-column label="房型列表" width="180" align="center">
				<template slot-scope="scope">
					<span>{{scope.row.houseList.toString()}}</span>
				</template>
			</el-table-column>
			<el-table-column label="出租时段" width="180" align="center">
				<template slot-scope="scope">
					<span>{{scope.row.chooseDate.toString().replace(",","至")}}</span>
				</template>
			</el-table-column>
			<el-table-column label="房屋描述" width="250" align="center">
				<template slot-scope="scope">
					<div style="height: 100px;overflow: scroll;text-align: left">{{scope.row.describe}}</div>
				</template>
			</el-table-column>
			<el-table-column label="房屋状态" width="100" align="center">
				<template slot-scope="scope">
			...
			<el-table-column fixed="right" label="操作" width="100" v-if="operation=='examine'">
				<template slot-scope="scope">
		...
		<el-pagination background layout="prev, pager, next" :page-size=pageSize :total="total"
			:current-page="currentPage" @current-change="page">
		</el-pagination>
	</article>
</template>
		...
		methods: {
			...
			async getHouseData(Page) { //得到房屋数据
				...
				if (this.operation == "seeAll") {
					...
					await axios.get(this.API.GetHouseByAdmin).then(resp => {
