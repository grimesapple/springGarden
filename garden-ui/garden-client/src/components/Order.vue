...
<template>
	<div style="width: 1100px; margin: 0 auto">
		<el-table :data="tableData" style="width: 100%" :header-cell-style="{
			...
			<el-table-column label="日期" width="250" style="text-align: center" align="center">
				<template slot-scope="scope">
					<i class="el-icon-time"></i>
					<span style="margin-left: 10px">{{ scope.row.startTime}} 至 {{ scope.row.endTime}}</span>
				</template>
			</el-table-column>
			<el-table-column label="房屋名称"style="text-align: center" align="center">
				<template slot-scope="scope">
			...
			<el-table-column prop="phone" label="用户手机号码" align="center" style="text-align: center">
			</el-table-column>
			...
			<el-table-column label="状态" align="center">
				<template slot-scope="scope">
					<span style="margin-left: 10px" v-if="scope.row.status==0">待支付</span>
					<span style="margin-left: 10px" v-if="scope.row.status==1">待入住</span>
					<span style="margin-left: 10px" v-if="scope.row.status==2">已离店</span>
					<span style="margin-left: 10px" v-if="scope.row.status==3">已取消</span>
				</template>
			</el-table-column>
...
<!-- 					<el-button v-if="type=='user'&&scope.row.state==2" size="mini"
		...
		<el-dialog :title="'评论'" :visible.sync="dialogVisible" width="30%" class="dialog">
			<div style="width:100%;">
				<p>{{currentTitle}}</p>
				<div style="text-align: left;margin-bottom: 10px">评论星级:</div>
				<el-rate v-model="score" style="float: left;margin-bottom: 10px"></el-rate>
				<el-input type="textarea" :rows="2" placeholder="请输入内容" v-model="describe">
				</el-input>
			</div>
	...
	export default {
		...
		methods: {
			...
			async handleCancel(index, row) { 
				...
				await axios.post(this.API.CancelPre, data).then((resp) => {
				...
				// 		"noticeType": "system",
				// 		'datetime': new Date,
				// 		'describe': "您在'" + row.title + "'的订单被用户取消了",
				// 		"url": ''
				// 	}
				// }
				...
				// 		"noticeType": "system",
				// 		'datetime': new Date,
				// 		'describe': "您在'" + row.title + "'的订单被房东取消了",
				// 		"url": ''
				// 	}
				// }
			...
			async getOrder() { //得到订单
				...
				if (this.type == "landlord") { //得到房东订单
					await axios.get(this.API.GetOrderList + this.$store.state.userInfo.username + '/null').then(
						function(resp) {
